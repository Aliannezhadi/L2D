# 📦 L2D — لینک به دانلود  
🌐 زبان‌ها: [English](README.md) | [فارسی](README-FA.md)

**L2D** یک اسکریپت PHP امن و سبک است که امکان دانلود فایل از طریق لینک و ذخیره آن در یک مسیر مشخص روی سرور را فراهم می‌کند.  
این ابزار برای **اتوماسیون، کرون‌جاب، سرورها و دانلودهای بک‌اند** عالی است.

---

## 🚀 ویژگی‌ها
- ✔ دانلود امن با اعتبارسنجی URL  
- ✔ جلوگیری از Path Traversal  
- ✔ محدودیت حجم فایل  
- ✔ مدیریت خطا  
- ✔ توکن امنیتی برای حفاظت  
- ✔ سازگار با **مرورگر** و **CLI**  
- ✔ سیستم لاگ‌نویسی  
- ✔ امکان محدود کردن هاست‌ها  
- ✔ دانلود با cURL و SSL  

---

## 📂 ساختار پوشه‌ها

```
/
├── downloads/              # فایل‌های دانلود شده
├── l2d.log                 # فایل لاگ
└── secure_downloader.php   # اسکریپت اصلی
```

---

## 🔧 تنظیمات اولیه

در فایل `secure_downloader.php` قابل تغییر است:

```php
const DOWNLOAD_DIR = __DIR__ . '/downloads';
const MAX_FILE_SIZE_BYTES = 50 * 1024 * 1024;
const SECURITY_TOKEN = 'CHANGE_ME_TO_A_LONG_RANDOM_TOKEN';
const ENABLE_LOGGING = true;
```

⚠️ **حتماً مقدار SECURITY_TOKEN را تغییر دهید.**

ساخت توکن قوی:

```bash
openssl rand -hex 32
```

---

# 🔐 امنیت توکن

## 1️⃣ استفاده در مرورگر
```
secure_downloader.php?url=https://example.com/file.zip&token=YOUR_TOKEN
```

## 2️⃣ استفاده در CLI (امن‌تر)
```
L2D_TOKEN=YOUR_TOKEN php secure_downloader.php "https://example.com/file.zip"
```

---

# 🛠 روش استفاده

### مرورگر:
```
?url=YOUR_URL&token=YOUR_TOKEN
```

### خط فرمان:
```
L2D_TOKEN=YOUR_TOKEN php secure_downloader.php "URL"
```

---

# 📝 سیستم لاگ

اگر فعال باشد، لاگ‌ها در:

```
l2d.log
```

ذخیره می‌شوند.

نمونه:

```
[2025-02-01 11:23:54] [OK] [CLI] URL="https://example.com/file.zip" EXTRA="SavedTo=/path/file.zip" MESSAGE="Download successful"
```

برای غیرفعال کردن:

```php
const ENABLE_LOGGING = false;
```

---

# 🕒 کرون‌جاب

اجرای اسکریپت هر ساعت:

```
0 * * * * L2D_TOKEN=YOUR_TOKEN php /path/to/secure_downloader.php "https://example.com/file.zip"
```

---

# 🧰 هاست‌های مجاز (اختیاری)

برای محدود کردن:

```php
const ALLOWED_HOSTS = ['example.com'];
```

برای آزاد بودن:

```php
const ALLOWED_HOSTS = [];
```

---

# 🧩 پیش‌نیازها

- PHP 7.4 یا بالاتر  
- فعال بودن cURL  
- فعال بودن SSL  
- دسترسی نوشتن روی پوشه Downloads  

